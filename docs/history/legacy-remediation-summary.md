...
-> OperationsCenter enforces policy and dispatches adapters
-> Observability records -> Tuning recommends improvements.
```

## Temporary shims

- No legacy execution runtime remains on the default path; the planning worker and canonical execute entrypoint now describe separate supported OperationsCenter boundaries.
